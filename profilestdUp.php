    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['profileimg'])) {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["profileimg"]["tmp_name"], $targetFile)) {
                // Update the user's profile image in the database based on username
                $update_query = "UPDATE students SET profileimg = ? WHERE username = ?";
                $update_stmt = $connect->prepare($update_query);
                $update_stmt->bind_param('ss', $targetFile, $username);
                $update_stmt->execute();
