...
if (isset($_SESSION['username'])) { 
    ...
    if ($gradeResult->num_rows == 1) {
        ...
        $sql = "SELECT * FROM assignmentsnew WHERE assigngrade = ? ORDER BY dateuploaded ASC";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<div>"."<a href='" .$row['assignfile']. "'>".$row['assignfile']."</a>"."</div>";
                  ...
                  // Form to upload the completed assignment
                  echo "<form action='stdcompletedassignup.php' method='POST' enctype='multipart/form-data'>";
                  echo "<input type='hidden' name='assignment_id' value='" . $row['id'] . "'>";
                  echo "<input type='hidden' name='assignname' value='" . $row['assignname'] . "'>";
                  echo "<input type='hidden' name='dateuploaded' value='" . $row['dateuploaded'] . "'>";
                  echo "<input type='hidden' name='assigntr' value='" . $row['assigntr'] . "'>";
                  echo "<input type='hidden' name='assigntype' value='" . $row['assigntype'] . "'>";
                  echo "<input type='hidden' name='assigngrade' value='" . $row['assigngrade'] . "'>";
                  echo "<input type='hidden' name='assignduedate' value='" . $row['assignduedate'] . "'>";
                  echo "<input type='hidden' name='assigninstructs' value='" . $row['assigninstructs'] . "'>";
                  echo "<input type='file' name='assignfile' required>";
