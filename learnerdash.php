         ...
         <main class="wrapper">
             <header class="dashboard">
                ...
                <img id="home-box-img" src="data:image/png;base64,">
                 ...
                 <a href="./learnerdash.php" style="text-decoration: none;" class="home_title home-title-home"> Welcome, 
                 <?php
                              ...
                              if (isset($_SESSION['username'])) {
                                  ...
                                  $stmt = $connect->prepare("SELECT name FROM students WHERE username = ?");
                                  $stmt->bind_param('s', $userName);
                                  $stmt->execute();
                                  $result = $stmt->get_result();
                 ...
                 <div class="request_btns">
                     ...
                     <a href="#" id="pay_req"><abbrev title="Current Class">
                     <?php 
                          // Check if the students ID is set in the session (user is logged in)
if (isset($_SESSION['username'])) {
  // Get the logged-in user's students ID from the session
  $loggedInusername = $_SESSION['username'];

  // Establish a database connection
  require("./connect.php");

  // Prepare SQL statement to fetch name based on the students ID
  $sql = "SELECT stdgrade FROM students WHERE username = ?";
  $stmt = $connect->prepare($sql);

  // Bind the students ID parameter to the SQL statement
  ...
  if ($result->num_rows > 0) {
      // Fetch the name associated with the students ID
      $row = $result->fetch_assoc();
      $stdgrade = $row['stdgrade'];

      // Display the name of the logged-in user
      echo "<span class='span-caps'>".$stdgrade."<span>";

      // You can use $studentsName for further processing/display
  } else {
...
if (isset($_SESSION['username'])) {
  ...
  // Prepare SQL statement to fetch name based on the students ID
  $sql = "SELECT status FROM students WHERE username = ?";
  $stmt = $connect->prepare($sql);
  ...
  if ($result->num_rows > 0) {
      // Fetch the name associated with the students ID
      $row = $result->fetch_assoc();
      $studentsStatus = $row['status'];

      // Display the name of the logged-in user
      echo "<span class='span-caps'>" . $studentsStatus. "</span>";
...
if (isset($_SESSION['username'])) {
    ...
    $loggedInusername = $_SESSION['username'];
    $getImageQuery = "SELECT profileimg FROM students WHERE username = ?";
    $getImageStmt = $connect->prepare($getImageQuery);

    if ($getImageStmt) {
        $getImageStmt->bind_param('s', $loggedInusername); // Assuming username is a string
     ...
     <g id="Lock-icon">
       ...
       <g clip-path="url(#mask_1)">
         ...
         <path d="M24 28C22.3431 28 21 29.3431 21 31C21 32.6568 22.3431 34 24 34C25.6568 34 27 32.6569 27 31C27 29.3432 25.6569 28 24 28L24 28Z" id="Shape" fill="#C76E00" stroke="none" />
             ...
             <!-- END IF - SIDE NAV FOR SCREENS > 996PX-->

             <!--START IF SIDE NAV FOR SCREENS BELOW 900PX -->
            
             <div class="hide-big studSideNav" id="studSideNav">
              <!--Close the staffsidenav -->
             <p id="closerStuder" class="closeStuder" onclick="closeStuder()">&times;</p>
                                  <!-- In the following containers we have an icon and label that link to a respective page-->
                 <p class="padder navbar default" onclick="openSec(event, 'profile')" id="opendefault">
                     <span class="icon" id="profile_settings_icon">
                        <!--<?xml version="1.0" encoding="utf-8"?>
        ...
        if ($getImageStmt->execute()) {
            ...
            if ($getImgResult) {
                if ($getImgResult->num_rows > 0) {
                    ...
                    // Display the image in HTML
                    echo '<img style="height:1.5pc; width: 1.5pc; border-radius: 100%;"  src="' . $imgPath . '" alt="Profile Image">';
                } else {
                    echo 'Profile image not found.';
                }
            } else {
     ...
     <g id="Lock-icon">
       ...
       <g clip-path="url(#mask_1)">
         ...
         <path d="M24 28C22.3431 28 21 29.3431 21 31C21 32.6568 22.3431 34 24 34C25.6568 34 27 32.6569 27 31C27 29.3432 25.6569 28 24 28L24 28Z" id="Shape" fill="#C76E00" stroke="none" />
             ...
             <button class="openStuder" id="openStuder" onclick="openStuder()">&#9776;</button>
              ...
              <!--Profile contents-->
                 ...
                 <div class="profileHolder">
                  <form method="post" action="#" class="prof_pic_box">
                    <abbrev id="show_user_name" title="<?php echo $_SESSION['username']; ?>"><a href="#">
                    <?php
// Start the session to access session variables
//session_start();
...
<form action="./profilestdUp.php" method="post" class="stud_upload_actions" enctype="multipart/form-data">
    ...
    if (isset($_SESSION['username'])) {
        ...
        // Prepare SQL statement to fetch details based on the students ID
        ...
        if ($result->num_rows > 0) {
            ...
            // Display students details if found
            echo "<p class='data-box'>Name: " . $studentDetails['name'] . "</p>";
            echo "<p class='data-box'>Registration Number: " . $studentDetails['username'] . "</p>";
            echo "<p class='data-box'>Stream: " . $studentDetails['stream'] . "</p>";
            echo "<p class='data-box'>Feebalance: " . $studentDetails['feebalance'] . "</p>";
            echo "<p class='data-box'>Date of Birth: " . $studentDetails['dateofbirth'] . "</p>";
            echo "<p class='data-box'>Gender: " . $studentDetails['gender'] . "</p>";
            echo "<p class='data-box'>Parent Phone: " . $studentDetails['parentphone'] . "</p>";
            echo "<p class='data-box'>Language: " . $studentDetails['language'] . "</p>";
            echo "<p class='data-box'>Nationality: " . $studentDetails['nationality'] . "</p>";

            // Add a data attribute to the "View More" link with the student ID
            echo "<p class='data-box'><a href='javascript:void(0);' class='view-more-link' data-student-id='{$loggedInusername}'>View More</a></p>";
            // Display other students details as needed
        } else {
            echo "<p>students details not found for students ID: $loggedInusername</p>";
        }

        // Close prepared statement
        $stmt->close();
    } else {
        echo "<p>User not logged in. Please log in to view the students details.</p>";
    }
...
<script>
        $(document).ready(function() {
            $(".view-more-link").click(function() {
                ...
                // window.location.href = "viewmore.php?studentID=" + studentID;
                // Open viewmore.php in a new tab
            window.open('viewstdmore.php?studentID=' + studentID, '_blank');
            });
        });
    </script>
                 ...
                 <div id="jobs" class="holdwrap job_holder job_settings">
                  ...
                  <div class="job_history boxer">
                    ...
                    <div class="regsubjbtns">
                      <button id="regsubjbtnmain" class="regsubjbtn" onclick="openregSubj(event, 'registerTable')">Register</button>
                      <button class="regsubjbtn" onclick="openregSubj(event, 'regSubjstable')">Registered</button>

                    </div>
                    <div class="regContainers">
                        <!--WRAPPER FORM TO REGISTER-->
                        <form id="registerTable"  action="./subjectstdreg.php" method="post" class="regtab upload_subj">
                            ...
                            <div class="table-diver-no table-diver-small">
                              ...
                              <div>
                                ...
                                <div>
                                  <div>
                                    ...
                                    <div>
                                      <!-- <input type="text" class="table_input" name="subjectonecode" id="subjoneCode"> -->
                                      <select name="subjectcode" class="exfilter sflexinp" id="statusftr">
                                        <option class="optionVal" value="" id="fhstatusdef" disabled selected>--Select Subject Code--</option>
                                        <option class="optionVal" value="Math0G1" id="fhstatusx">MATH0G1</option>
                                        <option class="optionVal" value="Math0G2" id="fhstatusy">MATH0G2</option>
                                        <option class="optionVal" value="Math0G3" id="fhstatusz">MATH0G3</option>
                                        <option class="optionVal" value="Math0G4" id="fhstatusz">MATH0G4</option>
                                        <option class="optionVal" value="Math0G5" id="fhstatusz">MATH0G5</option>
                                        <option class="optionVal" value="Math0G6" id="fhstatusz">MATH0G6</option>
                                        <option class="optionVal" value="Math0G7" id="fhstatusz">MATH0G7</option>
                                        <option class="optionVal" value="Math0G8" id="fhstatusz">MATH0G8</option>
                                        <option class="optionVal" value="Math0G9" id="fhstatusz">MATH0G9</option>
                                        <option class="optionVal" value="Eng0G1" id="fhstatusz">ENG0G1</option>
                                        <option class="optionVal" value="Eng0G2" id="fhstatusz">ENG0G2</option>
                                        <option class="optionVal" value="Eng0G3" id="fhstatusz">ENG0G3</option>
                                        <option class="optionVal" value="Eng0G4" id="fhstatusz">ENG0G4</option>
                                        <option class="optionVal" value="Eng0G5" id="fhstatusz">ENG0G5</option>
                                        <option class="optionVal" value="Eng0G6" id="fhstatusz">ENG0G6</option>
                                        <option class="optionVal" value="Eng0G7" id="fhstatusz">ENG0G7</option>
                                        <option class="optionVal" value="Eng0G8" id="fhstatusz">ENG0G8</option>
                                        <option class="optionVal" value="Eng0G9" id="fhstatusz">ENG0G9</option>

                                        <!--These codes will be created in the followinf syntax - subjectname in caps followed by grade-->
                                      </select>
        </div>
                              ...
                              <div>                                  
                                ...
                                <div>
                                  <div>
                                    <div>
                                      <!-- <input type="text" class="table_input" name="subjectonename" id="subjectoneName">  -->
                                      <select name="subjectname" class="exfilter sflexinp" id="statusftr">
                                        ...
                                        <option class="optionVal" value="Social-CRE" id="fhstatusz">S/S-CRE</option>
                                        ...
                                        <option class="optionVal" value="Creative Arts" id="fhstatusz">C/A</option>
                                      </select>
        </div>
        </div>
        </div>
                              ...
                              <div>
                                  <div>
                                    <div><input type="date" class="table_input" name="date" id="subjectoneupDate"></div>
                              ...
                              <div>
                                  <div>
                                    <div><input type="text" class="table_input" name="teacher" id="subjectoneTr"></div>
                                  </div>
                                </div>
                              </div>
...
<!-- ============================================================== --> 
                        ...
                        <!--WRAPPER FOR REGISTERED SUBJECTS-->
                            <div id="regSubjstable" class="regtab regSubjstable">
                            <?php
...
if (isset($_SESSION['username'])) {
  ...
  if ($result->num_rows > 0) {
      ...
      $row = $result->fetch_assoc();
      $studentGrade = $row['stdgrade'];

      // Display the name of the logged-in user
      echo "<p class='job-title span-caps'>$studentGrade</p>";

      // You can use $studentName for further processing/display
  } else {
      echo "Class not found.";
  }
                  ...
                  <div class="skills_section boxer">
                    ...
                    <div class="list_of_skills">
                     <?php
                            ...
                            // Your include statement
                            include 'regstdsubjects.php';
                            ?> 
                            <!-- <a href="./regsubjects.php">Regitered subjects</a> -->
                    </div>
                  </div>
<hr class="stuHr">
...
<!-- start if: assignments -->
                      <div class="subj-content_three subj_content" id="subjAssignments">
                        ...
                        <div class="stafftab_header">
                          <button id="stdasstabBtn" class="stdstaassbtn" onclick="openstdAsstab(event, 'availAss')">Available Assignment</button>
                          <button class="stdstaassbtn" onclick="openstdAsstab(event, 'uploadAss')">Upload Assignment</button>
                        </div>
                        <hr class="stuHr">
                        <div class="subj_cont_info">
                                <!--Two tabs for notes and uploading notes-->
                                <div id="uploadAss" class="stdasstabcont uploadt_section asst_tab">
                              <h3 class="upload_sec_title">Upload Assignments</h3>
                              <hr class="stuHr">

                              <div class="uploader_inner">
                                  <form action="./stdcompletedassignup.php" method="post" class="assuploader_form" id="uploaderForm" enctype="multipart/form-data">
                                ...
                                <div id="availAss" class="stdasstabcont tabforavnotes">
                            <!-- <p class="subj_content_text">There are currently no Assignments.</p> -->
                              <?php include('stdassignrecordsnew.php'); ?>
                          </div>
...
if (isset($_SESSION['username'])) {
  ...
  if ($result->num_rows > 0) {
      ...
      // Display the name of the logged-in user
      echo "<p class='job-title span-caps'>$studentGrade Completed Assignments</p>";
  ...
  echo "<p>User not logged in.</p>";
                  ...
                  <div class="job_history boxer">
                    ...
                    <div class="job_hist_table">
                      <!--Attach a php table generator file -->
                      <?php
                                // Enable error reporting
                                error_reporting(E_ALL);
                                ini_set('display_errors', 1);
                                ...
                                // Your include statement
                                include 'stdcompletedrecords.php';
                            ?>
                    </div>
                  </div>
...
<svg width="96px" height="96px" viewBox="0 0 96 96" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg">
  ...
  <use xlink:href="#img_1116" fill="#FFFFFF" stroke="none" />
         ...
         <script src="staffdash.js"></script> <!-- Why is it attached here? Consider moving the commands to learnerdash.js - it has to do with commands for the sidenav -->
         <script src="learnerdash.js">
    ...
    <script>
        ...
        //     function resetTimer() {
        //         clearTimeout(time);
        //         time = setTimeout(logout, 300000);  // 300000 ms = 5 minutes
        //     }
        // };
