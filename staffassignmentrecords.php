<?php
// List all available assignments uploaded by a specific teacher
...
$sql = "SELECT * FROM assignmentsnew WHERE staffid = ? ORDER BY dateuploaded ASC";
...
// Bind the logged-in staffid to the prepared statement
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<div id='editbtnbox'><button class='streceditbtn' onclick='editassRow(\"" . $row['assignname'] . "\")'>Edit</button> <button class='streceditbtn deleter_x' onclick='deleteassRow(\"" . $row['assignname'] . "\")'>Delete</button></div>";
        echo "</div>";
        echo "<br/>" . "<br/>";
        echo "<hr/>";
...
<script>
    function editassRow(assignname) {
    ...
    function deleteassRow(assignname) {
