...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $staffgrade = isset($_POST['staffgrade']) ? sanitize_input($_POST['staffgrade']) : '';
    ...
    if ($staffid && $staffname &&  $dateofbirth && $staffrole && $basicpay  && $department && $subjects && $staffemail && $staffphone && $nextofkin && $remedialall && $maritalstat && $yearofemp && $staffstatus && $staffgender && $nhifno && $nssfno && $tscno && $bankname && $bankaccno  && $nationality && $password && $confirmpassword) {
        ...
        if ($result->num_rows > 0) {
            echo "<script>
                    alert('Staff ID already exists. Please choose a different staff ID.');
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["profileimg"]["tmp_name"], $targetFile)) {
            ...
            $insert_stmt = $connect->prepare("INSERT INTO staff (staffid, name, dateofbirth, role,basicpay,staffgrade, stream,profileimg, department, subjects, email, phone, nextofkin, remedialallocation, maritalstatus, yearofemployment, status,gender, nhifno, nssfno,tscno,bankname,bankaccno, nationality, password)  
                                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )");
            ...
            if ($insert_stmt) {
                ...
                $insert_stmt->bind_param('sssssssssssssssssssssssss', $staffid, $staffname, $dateofbirth, $staffrole, $basicpay, $staffgrade, $stream, $targetFile, $department, $subjects, $staffemail, $staffphone, $nextofkin, $remedialall, $maritalstat, $yearofemp, $staffstatus, $staffgender, $nhifno, $nssfno, $tscno, $bankname, $bankaccno, $nationality, $hashed_password);
                $insert_stmt->execute();
