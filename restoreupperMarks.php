...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['username'])) {
    ...
    $sql_select = "SELECT * FROM deleteduppergrades WHERE username = ?";
    ...
    if ($result_select->num_rows > 0) { 
            ...
            $sql_insert = "INSERT INTO uppergrades (username, name, stdgrade, stream, subjmath, subjmathrub, subjeng, subjengrub, subjkisw, subjkiswrub, subjscie, subjscierub, subjsocial, subjsocialrub, subjscre, subjscrerub, subjca, subjcarub, subjagrinu, subjagrinurub, total)  
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt_insert = $connect->prepare($sql_insert);
    
            // Bind parameters and execute the query to insert into 'deletedlowergrades' table
            while ($row = $result_select->fetch_assoc()) {
                $stmt_insert->bind_param("ssssssssssssssssssssi", $row['username'], $row['name'], $row['stdgrade'], $row['stream'], $row['subjmath'], $row['subjmathrub'], $row['subjeng'], $row['subjengrub'], $row['subjkisw'], $row['subjkiswrub'], $row['subjscie'], $row['subjscierub'], $row['subjsocial'],  $row['subjsocialrub'], $row['subjscre'], $row['subjscrerub'], $row['subjca'], $row['subjcarub'], $row['subjagrinu'], $row['subjagrinurub'], $row['total']);
                $stmt_insert->execute();
            }
        ...
        if ($stmt_insert->affected_rows > 0) { 
            ...
            $sql_delete = "DELETE FROM deleteduppergrades WHERE username = ?";
